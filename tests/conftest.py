...
def mock_mailing_client(mocker):
    ...
    def mock_mail_api(email_recipients: list,
                      mail_sender: str,
                      subject: str,
                      text_body,
                      html_body=None):
...
def create_master_organization(test_client, initialize_database):
    organization = Organization(address='P.O.Box 112233', is_master=True, name='Test Master Organization')
...
def create_standard_organization(test_client, initialize_database):
    organization = Organization(address='P.O.Box 332211', is_master=False, name='Test Standard Organization')
    db.session.add(organization)
    db.session.commit()
    return organization
...
@pytest.fixture(scope='module')
def create_admin_user(test_client, initialize_database, seed_system_data, create_master_organization):
    organization = create_master_organization
    user = User(given_names='John Doe',
                surname='Jane',
...
def create_client_user(test_client, initialize_database, seed_system_data, create_master_organization):
    organization = create_master_organization
    user = User(given_names='Jon Snow',
                surname='Stark',
                phone='+254712345678',
