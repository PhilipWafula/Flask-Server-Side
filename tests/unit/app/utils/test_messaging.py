...
def test_send_one_time_pin(
    mock_sms_client, test_client, initialize_database, create_client_user
):
    ...
    assert f"Hello {client_user.given_names}, your activation code" in messages[0].get(
        "message"
    )
