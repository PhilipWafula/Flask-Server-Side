...
@pytest.mark.parametrize('mail_type, token_type, mail_subject', [
    ('user_activation', 'user_activation', 'Test Master Organization: Activate my account.'),
    ('reset_password', 'reset_password', 'Test Master Organization: Reset my password.')
])
def test_send_template_email(test_client,
                             initialize_database,
                             mock_mailing_client,
    ...
    mailing_client.send_template_email(mail_type=mail_type,
                                       token=token,
