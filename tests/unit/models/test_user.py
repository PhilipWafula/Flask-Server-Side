...
def test_user_password_change(test_client, initialize_database, create_admin_user, create_client_user):
    """
    GIVEN a user model
...
def test_valid_activation_token(create_admin_user):
    ...
    """
    activation_token = create_admin_user.encode_single_use_jws(token_type='user_activation')
    assert activation_token is not None
    validity_check = create_admin_user.decode_single_use_jws(token=activation_token,
...
def test_valid_authentication_token(activated_admin_user):
    ...
    assert (activated_admin_user.query.execution_options(show_all=True).filter_by(id=resp['id']).first() is not None)
