...
@pytest.mark.parametrize('name, is_master, expected_status_code', [
    ('Test Standard Organization', False, 201),
    ('Test Master Organization', True, 201),
])
def test_create_standard_organization(test_client, initialize_database, name, is_master, expected_status_code):
    """
    GIVEN a flask application
    WHEN a POST request with a valid standard organization json
    THEN check that organization is created.
    """
    response = test_client.post('/api/v1/organization/',
...
def test_get_all_organizations(test_client,
    ...
    response = test_client.get('/api/v1/organization/',
                               headers={
    ...
    if response.status_code == 200:
        assert len(response.json['data']['organizations']) == len(Organization.query.all())


def test_get_single_organization(test_client,
    ...
    """
    authentication_token = activated_admin_user.encode_auth_token().decode()
    response = test_client.get(f'/api/v1/organization/{create_master_organization.id}/',
                               headers={
                                   'Authorization': f'Bearer {authentication_token}',
                                   'Accept': 'application/json'
                               },
    ...
    assert response.status_code == 200
    if response.status_code == 200:
        assert response.json['data']['organization']['name'] == create_master_organization.name


def test_edit_organization_data(test_client,
                                create_master_organization,
                                activated_admin_user):
    """
    GIVEN a flask application
    ...
    response = test_client.put(f'/api/v1/organization/{create_master_organization.id}/',
                               headers={
                                   'Authorization': f'Bearer {authentication_token}',
                                   'Accept': 'application/json'
                               },
                               json={
                                   'name': 'Sample Master Organization'
                               },
                               content_type='application/json')
    assert response.status_code == 200
