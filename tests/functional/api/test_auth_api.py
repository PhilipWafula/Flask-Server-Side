...
@pytest.mark.parametrize('user_json, status_code', [(admin_user, 201), (client_user, 201)])
def test_register_api(test_client,
                      initialize_database,
...
def test_invalid_user_activation(test_client, create_admin_user, activation_token):
    ...
    response = test_client.post('/api/v1/auth/activate_user/',
                                headers={
                                    'Accept': 'application/json'
                                },
                                json={
...
malformed_otp_json = {'phone': '+254712345678', 'otp': 654871, 'otp_expiry_interval': 3600}


@pytest.mark.parametrize('otp_json, status_code', [
    (otp_json_with_missing_value, 400),
    (malformed_otp_json, 400)
])
def test_invalid_verify_otp(test_client, create_client_user, otp_json, status_code):
    ...
    response = test_client.post('/api/v1/auth/verify_otp/',
...
def test_resend_otp(test_client, initialize_database, seed_system_data, mock_sms_client):
    """
    GIVEN a flask application
    WHEN a POST request is sent to '/api/v1/auth/resend_otp/'
    THEN send back current OTP.
    """
    user = UserFactory(given_names='Arya Faceless',
                       surname='Stark',
                       phone='+254787654123',
    ...
    response = test_client.post('/api/v1/auth/resend_otp/',
                                headers={'Accept': 'application/json'},
                                json={
                                    'phone': '+254787654123'
                                },
    ...
    assert messages == [
        {'phone': f'{user.phone}', 'message': f'Hello {user.given_names}, your activation code is: {otp}'}
    ]
...
def test_login(test_client, activated_admin_user, activated_client_user):
    ...
    response = test_client.post('/api/v1/auth/login/',
                                headers={'Accept': 'application/json'},
                                json={
                                    'email': activated_admin_user.email,
                                    'password': 'password-123'
                                },
                                content_type='application/json')
    assert response.status_code == 200
    ...
    response = test_client.post('/api/v1/auth/login/',
                                headers={'Accept': 'application/json'},
                                json={
                                    'phone': activated_client_user.phone,
                                    'password': 'password-123'
                                },
                                content_type='application/json')
    assert response.status_code == 200
...
def test_request_password_reset_email(test_client, activated_admin_user):
    ...
    """
    response = test_client.post('/api/v1/auth/request_password_reset_email/',
                                headers={'Accept': 'application/json'},
                                json={
                                    'email': activated_admin_user.email
                                },
...
def test_password_reset(test_client, activated_admin_user):
    ...
    """
    password_reset_token = activated_admin_user.encode_single_use_jws(token_type='reset_password')
    authentication_token = activated_admin_user.encode_auth_token().decode()
